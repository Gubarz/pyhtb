...
def get_all_servers(sdk: PyHTB, product: GetConnectionsServersProduct):
    ...
    for loc_name, loc_prop in options.additional_properties.items():
        for group_name, group_obj in loc_prop.additional_properties.items():
            if not group_obj.servers:
                continue
            for srv_key, srv_obj in group_obj.servers.additional_properties.items():
                servers.append({
                    "id": srv_obj.id,
...
def find_vpn_server(sdk: PyHTB, target: str):
    ...
    for pool in pools:
        try:
            ...
            for s in servers:
                if str(s["id"]) == target or s["name"].lower() == target.lower():
                    return s
        except Exception:
            pass
    return None
...
def vpn_status():
    ...
    try:
        ...
        rows = []
        # Response is list[ConnectionStatusItem]
        for item in response:
            ...
            rows.append([
                ...
                ip,
                loc,
                getattr(item, "connection_type", "N/A")
            ])
            
        headers = ["Server Name", "IP Address", "Location Type", "Connection Type"]
        formatter.render_table("Active Connections", headers, rows)
    except Exception as e:
        formatter.print_error(f"Failed to retrieve status: {e}")

@vpn_group.command(name="servers", help="List available VPN servers")
@click.option("--pool", type=click.Choice(["labs", "starting_point", "competitive"], case_sensitive=False), default="labs", help="Server pool to list (default: labs)")
def vpn_servers(pool):
    ...
    try:
        ...
        formatter.print_error(f"Failed to list servers: {e}")
...
def vpn_switch(target):
    ...
    if not server:
        formatter.print_error(f"Could not resolve server '{target}' to a valid VPN server ID.")
        return
    ...
    try:
        ...
        if detailed.status_code == 200:
            ...
            formatter.print_api_error("switch server", detailed)
    except Exception as e:
        formatter.print_error(f"Failed to switch server: {e}")
...
def vpn_download(target, tcp, output):
    ...
    try:
        ...
        if detailed.status_code != 200:
            formatter.print_api_error("download config", detailed)
            return
        ...
        if res and hasattr(res, "payload"):
            ...
            if hasattr(payload, "getvalue"):
                ...
                ovpn_data = val.decode() if isinstance(val, bytes) else val
            elif hasattr(payload, "read"):
                val = payload.read()
                ovpn_data = val.decode() if isinstance(val, bytes) else val
                
        if not ovpn_data:
            formatter.print_error("Empty or invalid config payload received.")
            return
        ...
        formatter.print_error(f"Failed to download config: {e}")
