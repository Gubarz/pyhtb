from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="SpTiersProgressItem")


@_attrs_define
class SpTiersProgressItem:
    """
    Attributes:
        completion_percentage (int | Unset):
        description (str | Unset):
        free_machine_completion_percentage (int | Unset):
        id (int | Unset):
        name (str | Unset):
    """

    completion_percentage: int | Unset = UNSET
    description: str | Unset = UNSET
    free_machine_completion_percentage: int | Unset = UNSET
    id: int | Unset = UNSET
    name: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        completion_percentage = self.completion_percentage

        description = self.description

        free_machine_completion_percentage = self.free_machine_completion_percentage

        id = self.id

        name = self.name

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if completion_percentage is not UNSET:
            field_dict["completion_percentage"] = completion_percentage
        if description is not UNSET:
            field_dict["description"] = description
        if free_machine_completion_percentage is not UNSET:
            field_dict["free_machine_completion_percentage"] = (
                free_machine_completion_percentage
            )
        if id is not UNSET:
            field_dict["id"] = id
        if name is not UNSET:
            field_dict["name"] = name

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        completion_percentage = d.pop("completion_percentage", UNSET)

        description = d.pop("description", UNSET)

        free_machine_completion_percentage = d.pop(
            "free_machine_completion_percentage", UNSET
        )

        id = d.pop("id", UNSET)

        name = d.pop("name", UNSET)

        sp_tiers_progress_item = cls(
            completion_percentage=completion_percentage,
            description=description,
            free_machine_completion_percentage=free_machine_completion_percentage,
            id=id,
            name=name,
        )

        sp_tiers_progress_item.additional_properties = d
        return sp_tiers_progress_item

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
